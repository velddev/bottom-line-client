      ...
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold tracking-widest">
            ...
            <span className="text-gray-400">MMO</span>
          </h1>
          <p className="text-gray-600 text-xs mt-2 tracking-widest uppercase">Open World Economy</p>
        </div>

        {/* Tab toggle */}
        <div className="flex border border-gray-800 rounded mb-6 text-sm">
          {(['register', 'login'] as const).map((m) => (
            <button
              key={m}
              onClick={() => setMode(m)}
              className={`flex-1 py-2 rounded transition-colors capitalize ${mode === m ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white'}`}
            >
        ...
        {mode === 'register' ? (
          <form onSubmit={(e) => { e.preventDefault(); register.mutate(); }} className="space-y-4">
            <div>
              ...
              <input
                className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-2.5 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-indigo-500"
            ...
            <button
              ...
              className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white py-2.5 rounded text-sm font-semibold transition-colors"
            >
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            {[
              ...
              <div key={label}>
                ...
                <input
                  className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-2.5 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-indigo-500 font-mono"
                  value={val}
